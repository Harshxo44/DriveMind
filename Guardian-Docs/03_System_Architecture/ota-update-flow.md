# Guardian - OTA Update Flow

> **Status:** Draft foundation | **Owner:** TBD | **Review cadence:** monthly and at each design gate

## Purpose

Define the decisions, evidence, and acceptance criteria for **OTA Update Flow** so the Guardian team can move from discovery to a safe, testable product decision.

## Scope

This document covers safe boundaries between device, app, cloud, AI, and the vehicle network. It applies to the India-first MVP for mid-range and older passenger vehicles; it excludes direct vehicle actuation, safety-critical driving control, and unsupported vehicle modifications.
...
|---|---|---|
| Experience | Driver and owner outcome for OTA Update Flow | Safe, understandable, low-friction |
...
sequenceDiagram
  ...
  D->>C: Check signed release manifest
  C-->>D: Compatible image and signature
  D->>D: Verify, install inactive slot, reboot
  D-->>M: Report health check or rollback
```
...
|---|---|---|
| 03-OTA-UPDATE-FLOW-01 | Outcome and owner are explicit | Reviewed specification and accountable DRI |
| 03-OTA-UPDATE-FLOW-02 | Failure behavior protects the driver and vehicle | Test record, safe-state definition, and support playbook |
| 03-OTA-UPDATE-FLOW-03 | Data handling follows consent and retention policy | Privacy/security sign-off and audit trail |
...
|---|---|---|---|
| 0.1 | 2026-08-03 | Guardian documentation bootstrap | Initial engineering template tailored to OTA Update Flow |
