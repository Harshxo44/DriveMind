# Guardian - Overall Architecture

> **Status:** Draft foundation | **Owner:** TBD | **Review cadence:** monthly and at each design gate

## Purpose

Define the decisions, evidence, and acceptance criteria for **Overall Architecture** so the Guardian team can move from discovery to a safe, testable product decision.
...
|---|---|---|
| Experience | Driver and owner outcome for Overall Architecture | Safe, understandable, low-friction |
...
```mermaid
flowchart LR
  V["Vehicle OBD-II / CAN"] --> D["Guardian device"]
  D <--> M["Mobile app"]
  D <--> C["Guardian cloud"]
  M <--> C
  C --> A["AI services"]
  C --> O["Telemetry, reminders, OTA"]
```
...
|---|---|---|
| 03-OVERALL-ARCHITECTURE-01 | Outcome and owner are explicit | Reviewed specification and accountable DRI |
| 03-OVERALL-ARCHITECTURE-02 | Failure behavior protects the driver and vehicle | Test record, safe-state definition, and support playbook |
| 03-OVERALL-ARCHITECTURE-03 | Data handling follows consent and retention policy | Privacy/security sign-off and audit trail |
...
|---|---|---|---|
| 0.1 | 2026-08-03 | Guardian documentation bootstrap | Initial engineering template tailored to Overall Architecture |
