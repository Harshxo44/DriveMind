# Guardian - API Error Model

> **Status:** Draft foundation | **Owner:** TBD | **Review cadence:** monthly and at each design gate

## Purpose

Define the decisions, evidence, and acceptance criteria for **API Error Model** so the Guardian team can move from discovery to a safe, testable product decision.

## Scope

This document covers versioned contracts, device-safe retries, compatibility, and observable failures. It applies to the India-first MVP for mid-range and older passenger vehicles; it excludes direct vehicle actuation, safety-critical driving control, and unsupported vehicle modifications.
...
|---|---|---|
| Experience | Driver and owner outcome for API Error Model | Safe, understandable, low-friction |
...
|---|---|---|
| 10-API-ERROR-MODEL-01 | Outcome and owner are explicit | Reviewed specification and accountable DRI |
| 10-API-ERROR-MODEL-02 | Failure behavior protects the driver and vehicle | Test record, safe-state definition, and support playbook |
| 10-API-ERROR-MODEL-03 | Data handling follows consent and retention policy | Privacy/security sign-off and audit trail |
...
|---|---|---|---|
| 0.1 | 2026-08-03 | Guardian documentation bootstrap | Initial engineering template tailored to API Error Model |
